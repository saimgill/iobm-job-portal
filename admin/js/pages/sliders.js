...
$(document).ready(function() {
    ...
    var price = 80;
    var currentslide=1000;
    var changedslide;
	$("#slider-show-val").on("slide", function(slideEvt) {
        changedslide = slideEvt.value;
        if(changedslide > currentslide){
            price += 30;
            currentslide = changedslide;
        }
        else if(changedslide < currentslide){
            price -= 30;
            currentslide = changedslide;
        }
        
        if(slideEvt.value > 0  && slideEvt.value <= 25000){
            $("#sum").text( slideEvt.value+' ');
            $(".pricelistings").text('$ ' + price);
        }
